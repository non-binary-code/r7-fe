...
import { filterItems, getRecycleItems } from '../utils/api';
...
const Recycle = () => {
  ...
  useEffect(() => {
    getRecycleItems().then((res) => setRecycleItems(res));
  }, []);
  ...
  return (
    <div className='bg-gray-100 h-[100vh]'>
      ...
      <div className='flex flex-wrap justify-center gap-10 items-center p-5'>
        {recycleItems?.slice(0, visible)?.map((item) => (
      ...
      <div className='flex justify-center items-center'>
        <p style={{ color: 'black', marginTop: '20px', marginBottom: '-20px' }}>
          {visible <= recycleItems.length
            ? `${visible}/${recycleItems.length} items showing`
            : `${recycleItems.length}/${recycleItems.length} items showing`}
        </p>
        {visible < recycleItems.length && (
